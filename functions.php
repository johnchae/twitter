<?php
	...
	$link = mysqli_connect("shareddb-f.hosting.stackcp.net", "twitterdb-3639798c", "wdttjl05h", "twitterdb-3639798c");
	...
	function displayTweets($type) {
		...
		if (mysqli_num_rows($result) == 0) {
			...
			while ($row = mysqli_fetch_assoc($result)) {
				...
				$userQueryResult = mysqli_query($link, $userQuery);
				$user = mysqli_fetch_assoc($userQueryResult);
				echo "<div class='tweet'><p><a href='?page=publicprofiles&userid=" . $user['id'] . "'>" . $user['email'] . "</a> <span class='time'>" . timeSince(time() - strtotime($row['datetime'])) . " ago</span>:</php>";
				echo "<p>" . $row['tweet'] . "</p>";
